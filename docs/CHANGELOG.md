...
* BUGFIX: [vmalert](https://docs.victoriametrics.com/vmalert.html): set `ActiveAt` timestamp of created alerts to the execution timestamp instead of the actual timestamp when rule was executed. The change is justified by [alert generator specification](https://github.com/prometheus/compliance/blob/main/alert_generator/specification.md#pending-state-conditions): "evaluation time when the alert is first created is referred to as ActiveAt for that alert".
